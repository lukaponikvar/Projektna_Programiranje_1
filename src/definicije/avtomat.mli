...
val dodaj_prehod : Stanje.t -> int -> char -> Stanje.t -> int option -> t -> t
val dodaj_prazen_prehod : Stanje.t -> int -> Stanje.t -> int option -> t -> t
val prehodna_funkcija : t -> Stanje.t -> int -> char -> (Stanje.t) option
val prehodna_funkcija_za_sklad : t -> Stanje.t -> int -> char -> int option
val zacetno_stanje : t -> Stanje.t
val seznam_stanj : t -> Stanje.t list
val seznam_prehodov : t -> (Stanje.t * int * char * Stanje.t * int option ) list
val seznam_praznih_prehodov : t -> (Stanje.t * int * Stanje.t * int option) list
val sklad : t -> Sklad.t
